...
class ToTensor(object):
    def __call__(self, pic):
        return F.to_tensor(pic[0]), F.to_tensor(pic[1])
...
class Normalize(object):
    ...
    def __call__(self, tensor):
        return F.normalize(tensor[0], self.mean, self.std, self.inplace), F.normalize(tensor[1], self.mean, self.std, self.inplace)
