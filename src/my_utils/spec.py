...
class SpecDataset(torch.utils.data.Dataset):
    def __init__(self, opt, datadir, tokenizer, dirA='PSD_Train_specular', dirB='PSD_Train_diffuse',imgsize=None):
    ...
    def __getitem__(self, index):
        ...
        return {
            "output_pixel_values": F.normalize(torch.from_numpy(T), mean=[0.5], std=[0.5]),  # Normalize GT
            "conditioning_pixel_values": torch.from_numpy(M),  # No normalization for input
            "caption": caption,
            "input_ids": input_ids,
            "image_name": fnA[:-4],
            "mask": mask
...
class GroupDataset(torch.utils.data.Dataset):
    def __init__(self, opt,datadir, tokenizer, imgsize=None,groups=95,idxs=12,idxd=1,idxis=[7],name="group-{:04d}-idx-{:02d}.png",freq=-1,any_valid=True):
...
#     def __init__(self, datadir, tokenizer, imgsize='small'):
...
#         return {
#             "output_pixel_values": F.normalize(torch.zeros([1,0]), mean=[0.5], std=[0.5]),  # Normalize GT
#             "conditioning_pixel_values": M,  # No normalization for input
#             "caption": caption,
#             "input_ids": input_ids,
#             "image_name": fn[:-4]
#         }         
...
class TestDataset(torch.utils.data.Dataset):
    def __init__(self, datadir, tokenizer, dirA='PSD_Test_specular', dirB='PSD_Test_diffuse',imgsize='small'):
        super(TestDataset, self).__init__()
        self.datadir = datadir
        self.dirA = dirA
        self.dirB = dirB
...
class TestGroupDataset(torch.utils.data.Dataset):
    def __init__(self, datadir, tokenizer, imgsize=None,groups=95,idxs=12,idxd=1,idxis=[7],name="group-{:04d}-idx-{:02d}.png",freq=-1,any_valid=True):
        super(TestGroupDataset, self).__init__()
        self.datadir = datadir
        self.imgsize = imgsize
        self.freq = freq
        self.tokenizer = tokenizer         
