...
from SynthesisDark import SynthesisNet
...
def initialize_vae(rank=4, return_lora_module_names=False):
    ...
    vae = AutoencoderKL.from_pretrained("stabilityai/sd-turbo", subfolder="vae", in_channels=1, out_channels=1, low_cpu_mem_usage=False, ignore_mismatched_sizes=True)
...
class CycleGAN_Turbo(torch.nn.Module):
    ...
    def load_ckpt_from_state_dict(self, sd):
        ...
        self.init_pred_net = SynthesisNet(batch_size=1)
        # create new OrderedDict that does not contain `module.`
        from collections import OrderedDict
        new_state_dict = OrderedDict()
        for k, v in sd["sd_init_pred"].items():
            name = k[7:] # remove `module.`
            new_state_dict[name] = v    
        self.init_pred_net.load_state_dict(new_state_dict)
        # self.init_pred_net.load_state_dict(sd["sd_init_pred"])
    ...
    def forward_with_networks(x, direction, vae_enc, unet, vae_dec, sched, timesteps, text_emb):
        ...
        # print("x_out_decoded.shape", x_out_decoded.shape)
